...
class P0:  # 创建类
    # self.flag与0比较大小时候的标记z是否为0的记号
    def __init__(self):  # 初始化
        # 导入CT图像
        self.CT0 = np.load("./CT0.npy")
        # 导入第一层卷积核权重矩阵
        self.convolution_weight = np.load("./convolution0_weight.npy")
        # 导入第一层卷积核偏置
        self.convolution_bias = np.load("./convolution0_bias.npy")
        # 这里非常粗暴，直接将self.convolution_col置为5##########################
        self.convolution_row = 5
        self.convolution_col = 5
        self.convolution_depth = 32  # 32*1*5*5
        self.convolution_number = self.convolution_row*self.convolution_col
        ######################################################################

        # 导入第三层卷积核权重矩阵
        self.convolution_weight_3 = np.load("./convolution0_weight_3.npy")
        # 导入第三层卷积核偏置
        self.convolution_bias_3 = np.load("./convolution0_bias_3.npy")

        # 导入全连接层权重矩阵(10,3136)
        self.fc_weight = np.load("./fc_weight_0.npy")
        # 导入全连接层bias(10,)
        self.fc_bias = np.load("./fc_bias_0.npy")
        # 导入比较三元组
        self.compare_triple = np.load("./compare_triple0.npy")
        # 导入乘法三元组
        self.multiplication_triple = np.load("./multiplication_triple0.npy")
        # 初始化特征图的长和宽

        self.feature_map = self.CT0
        ...
        self.new_compare_number = 0
        self.new_multiplication_number = 0
    ...
    def expand(self, matrix, convolution_matrix):
        ...
        # print(self.expansion_feature_map.shape)#(19600, 1)
        # 展开图像
        ...
        # print(self.expansion_feature_map.shape)#(627200, 1)
        # 展开卷积核
        ...
        # print(self.expansion_convolution.shape)#(19600, 1)
        for k in range(self.convolution_depth-1):
            ...
            self.expansion_convolution = np.vstack(
                [self.expansion_convolution, temp])
        # print(self.expansion_convolution.shape)#(627200,1)
    ...
    def expand_3d(self, matrix, convolution_matrix):
        ...
        # print(self.expansion_convolution.shape)  # (10035200, 1)
    ...
    def compute_e_f(self, matrix_x, matrix_y):  # 计算e=x-a,f=y-b要求：
        ...
        # 保存matrix_e_f0
        np.save("./matrix_e_f0.npy", matrix_e_f)
        self.matrix_e_f0 = matrix_e_f

    def compute_z(self):  # 第一层卷积层的乘法计算z
        matrix_e_f1 = np.load("./matrix_e_f1.npy")
        matrix_e_f = np.zeros(matrix_e_f1.shape)
        matrix_e_f = self.matrix_e_f0 + matrix_e_f1
        self.z = matrix_e_f[:, 1]*self.multiplication_triple[self.multiplication_number:self.multiplication_number+matrix_e_f1.shape[0], 0] + matrix_e_f[:, 0] * \
            self.multiplication_triple[self.multiplication_number:self.multiplication_number+matrix_e_f1.shape[0], 1] + \
            self.multiplication_triple[self.multiplication_number:
                                       self.multiplication_number+matrix_e_f1.shape[0], 2]
        self.new_multiplication_number += matrix_e_f1.shape[0]

        # self.multiplication_number += matrix_e_f1.shape[0]

        return self.z

    def output_z(self):
        np.save("./z0.npy", self.z)
    ...
    def init_s_mul(self):
        ...
        self.new_compare_number += self.z.shape[0]

        for i in range(self.s_mul.shape[0]):
            if abs(self.z[i, 0]) < 0.00000001:
                self.flag[i, 0] = 1
                self.s_mul[i, 0] = 0
            elif self.compare_triple[i, 1] > 0:
                self.s_mul[i, 0] = 1
            elif self.compare_triple[i, 1] < 0:
                self.s_mul[i, 0] = -1
    ...
    def init_rand(self):
        ...
        self.s_mul_0_add_0 = self.s_mul-rand

        np.save("./s_mul_0_rand.npy", rand)

    def get_rand(self):
        self.s_mul_1_add_0 = np.load("./s_mul_1_rand.npy")
        self.s_mul_1_add_0 = self.s_mul_1_add_0.reshape(-1, 1)
    ...
    def sum_convolution_3d(self):  # 计算卷积层后的特征图
        # print((self.feature_map_row-self.convolution_col+1)*(self.feature_map_col-self.convolution_col+1) * self.convolution_number)
        self.feature_map = np.zeros(
            (self.convolution_depth, self.feature_map_col-self.convolution_col+1, self.feature_map_row-self.convolution_col+1))
        for k in range(self.convolution_depth):
            for i in range(self.feature_map_row-self.convolution_col+1):
    ...
    def expansion_fully_connection(self):
        ...
        print(self.expansion_fully_connection_fc_weight.shape)
        print(self.expansion_fully_connection_feature_map.shape)
