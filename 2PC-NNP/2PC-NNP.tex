...
\begin{abstract}
    ...
    % 为了数据拥有者的隐私和模型提供方模型参数的安全，需要采用安全多方计算、同态加密等技术在保护隐私的情况下正确运行深度学习的模型，
    % 实现对图片的隐私保护。目前使用的方法都使用了非对称公钥加密体系，导致运行速度较慢。
    ...
    We propose 2PC-NNP, a two-party computation (2PC) framework for efficient neural network predictions.%evaluating neural network secretly. 
...
\section{Introduction}
    ...
    \begin{itemize}
        ...
        %     Convolution operation can be converted into a matrix multiplication and 
        %     the 2PC-convolution can be implemented by 
        %     2PC-matrix multiplication.
        %     Furthermore, we propose an optimization of 2PC-convolution, which
        ...
        %     while 2PC-maximum pooling involves 2PC-maximum protocol.
        ...
        % We propose the $\mathcal{S}-\mathcal{S}$ setting where
        % the data owner and the model provider outsource the prediction task to two independent servers.
        % After two servers running the whole process of 2PC-NNP,
        % they return their shares of classification label to the model provider and the data owner.
...
\section{Preliminaries}
    ...
    % Convolution is an operation between the imput image $X$ and the filter. 
    % Parameters of the filter includes weight tensor $W$
    % and bias vector $\overrightarrow{b}$.
    % It computes the dot product of the filter and the corresponding submatrix in the input image.
    % This step is repeated by sliding the filter to get the output matrix $Y$.
    % To improve the efficiency, convolution is usually converted to matrix multiplication and addition.
    % $$Y=X'\cdot W'+B'$$ 
    % Where $X'$, $W'$ and $B'$ are rearranged matrix of the input image $X$, weight tensor $W$
    % and bias vector $\overrightarrow{b}$. 
    ...
    % When running $\mathtt{multiplication}$ protocol, 
    ...
    \begin{table}[!ht]
        ...
        ABY&                   18$l$& $7l\cdot \kappa+(l^{2}+l)/2$ &4\\\hline 
    ...
    % Detailed framework of Lenet-5 and Resnet50 is shown in Fig .
...
\end{document} 
    ...
    % So the client-server ($\mathcal{C}$-$\mathcal{S}$) setting is introducted, 
    % where the client and server evaluate neural network prediction together
    % via some techniques such as the Beaver multiplication, the oblivious transfer protocol and the garbled circuit.
    % In classical MPC 
    % the Beaver designed the efficient multiplication 
    % To avoid using
    % These techniques also involves asymmetric cryptography operations, 
    % while it costs less due to transferring blinded plaintext
    % instead of ciphertext during the multiplication period.
    % Though the overhead of multiplication can be reduced much,
    % the efficiency of comparison and piecewise functions is still unsatisfactory
    % due to the extensive use of OT protocols and garbled circuits.

    % The most common used setting is the client-server ($\mathcal{C}-\mathcal{S}$) setting, 
    % where the data owner take the role as the client and the model provider as the server.
    % However, in practice, most of the data owners like the IOT equipments and some samll companies
    % are not able to carry out the 
    % whole process of 2PC neural network due to poor computational resource and lack of 
    ...
    % where data owner and the model provider outsource the prediction task
    % to two isolate servers. 
    ...
    % the first server and the second server belongs to different company.
    % In terms of techniques, the ($\mathcal{S}-\mathcal{S}$) setting has little difference compared to the ($\mathcal{C}-\mathcal{S}$) setting.
    % If needed, the ($\mathcal{S}-\mathcal{S}$) setting can degenerate into the ($\mathcal{C}-\mathcal{S}$) setting
    % by the data owner and the model provider taking the role as two servers directly.
