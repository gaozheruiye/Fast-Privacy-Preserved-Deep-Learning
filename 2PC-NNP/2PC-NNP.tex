...
\section{Related Work}
    ...
    The most common idea is to utilize fully homomorphic encryption  \cite{Homomorphic1} which supports multiplication and addition on ciphertext.
...
\section{Preliminaries}
    ...
    Note that, $sign(x\cdot y)=sign(x)\cdot sign(y)$.
    $$sign(x)=\begin{cases}
        1 & \text{ if } x>0 \\
        0 & \text{ if } x=0\\
        -1 & \text{ if } x<0
    ...
    % A secure two-party computation protocol enables players to evaluate a public function
    % with two private confidential inputs from two players
    % without disclosing more information than what can be inferred from the result.
    % We use `2PC' to stand for `secure two parties computation'.
    % Two parties running 2PC protocols are denoted as $\mathcal{P}_{0} $ and $\mathcal{P}_{1}$.
    ...
    \begin{itemize}
        ...
        % &ReLU(x) = max(0,x)\\
        &Maxout: f(x) = max( \overrightarrow{\mathbf{x}})\\
