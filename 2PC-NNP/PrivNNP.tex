...
\section{Two-party Secure Computation for Basic Operations}
    ...
    the input and the output are in the form of additive shares.
    ...
    Because $(\llbracket u \rrbracket^{0},z)$ and $\llbracket u \rrbracket^{1}$
    ...
    In view of $d_{0}=-\infty $ and $d_{n}=+\infty$,
...
\subsection{2PC protocols for neural network layers}
    ...
    $\langle \mathsf{ReLU}(\langle x\rangle)\rangle=\frac{(1+\langle \mathsf{sign}(\langle x\rangle)\rangle)\otimes \langle x\rangle}{2}$.
   ...
   Among these schemes, PrivNNP performs best in all respects.
    ...
    For accurate and stable measurement, file transferring rate between servers is fixed at 10M/s.
    ...
    the time consumption of online communication increases linearly with the number of elements,
    ...
    \begin{table}[!ht]
        ...
        \resizebox{0.45\textwidth}{!}{
        \begin{tabular}{c||c|c|c|c}\hline
        \multirow{2}{*}{Model}& \multicolumn{3}{c|}{Mime(s)}&Communication  \\
        \cline{2-4}
