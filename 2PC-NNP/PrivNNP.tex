...
\section{Introduction}
    ...
    \begin{itemize}
        ...
        we implement PrivNNP and conduct \textbf{experiment} on the classical
...
\section{Preliminaries}
    ...
    \textbf{Multi-Party Computation and Two-Party Computation:}
    ...
    % \begin{itemize}
    %     \item The ideal world/real world paradigm \cite{SecurityProof}:
    ...
    %     \item Let $\pi $ be a deterministic 2PC protocol for computing
    %     $f=(f_{0},f_{1})$,
    ...
    %     \item A 2PC protocol for $f$ is secure in the presence of semi-honest adversaries
    %     if there exist simulators $\mathcal{S}_{0}$ and $\mathcal{S}_{1}$ such that
    %     the view of adversary in real world is indistinguishable to that in ideal world.
    %     Namely, $\mathsf{view}_{i}^{\mathcal{S}_{i}}$ is indistinguishable with
    %     $\mathsf{view}_{i}^{\pi}$.

    % \end{itemize}

    \textbf{2PC secret sharing:}
    ...
    is denoted as $\mathsf{view}_{0}^{\mathcal{S}_{0}}=(\langle \mathbf{x}\rangle ^{i};m^{i}_{1},...,m^{i}_{t})$.
    ...
    \textbf{Layers of neural networks:}
...
\section{Two-party Secure Computation for Basic Operations}
    ...
    % \textbf{Definition of security for 2PC protocols}:
    % \begin{itemize}
    %     \item The ideal world/real world paradigm \cite{SecurityProof}:
    %     in real world, parties interact following the protocol,
    %     while in ideal world, parties have access to a
    ...
    %     Namely, $\mathsf{view}_{i}^{\mathcal{S}_{i}}$ is indistinguishable with
    %     $\mathsf{view}_{i}^{\pi}$.
    %     % and $\mathsf{view}_{1}^{\mathcal{S}_{1}}$ is indistinguishable with
    %     % $\mathsf{view}_{1}^{\pi}$.
    % \end{itemize}
...
\subsection{2PC protocols for neural network layers}
    ...
    %     &Sharemind&               0&     6$l^{2}$               &$l$\\\cline{2-5}
    %     &PrivNNP&                   0&   5$l$                  &3 \\\hline

    %     &ABY&                   54$l$& $21l\cdot \kappa+(l^{2}+l)3/2+12$ &16\\\cline{2-5}
    %     pooling&Sharemind&               0&     18$l^{2}$ +12$l$              &$2l+4$\\\cline{2-5}
    %     &PrivNNP&                   0&   27$l$                  &10 \\\hline

    %     \multirow{3}{*}{ReLU}&ABY&                   18$l$& $7l\cdot \kappa+(l^{2}+l)/2+2$ &7\\\cline{2-5}
    %     &Sharemind&               0&     $6l^{2}+2l$             &$l+1$\\\cline{2-5}
    %     &PrivNNP&                   0&   7$l$                  &4 \\\hline


    %     \end{tabular}}
    %     \caption{Overhead of different 2PC schemes with $l$-bits value inputs and  $\kappa$ security parameters }
    ...
    \begin{table}[!ht]
        ...
        \multirow{2}{*}{2PC task}& \multirow{2}{*}{schemes} &asymmetric & communication & \multirow{2}{*}{rounds}     \\
        &   &    computation       &     (bits)      &            \\ \hline
        \multirow{3}{*}{comparison}&ABY&                   $O(l)$& $O(l\cdot \kappa)$ &$O(\mathsf{log} l)$\\\cline{2-5}
        &Sharemind&               0&     $O(l^{2})$              &$O(l)$\\\cline{2-5}
        &PrivNNP&                   0&   $O(l)$                  &3 \\\hline

        \end{tabular}}
        \caption{Asymptotic complexities of different 2PC-comparison schemes with $l$-bits value inputs and  $\kappa$ is the asymmetric security parameters }
        \label{Time of methods}
    \end{table}
    ...
    \begin{table}[!ht]
        ...
        \end{tabular}}
        \caption{Overhead of different 2PC schemes with $32$-bits value inputs and  $\kappa$ is the asymmetric security parameters }
        \label{Overhead of methods}
    \end{table}
