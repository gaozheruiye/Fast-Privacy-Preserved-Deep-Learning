...
\section{Introduction}
    ...
    speech recognition, natural language process \cite{NLP}, machine translation, etc. Recently,
    deep learning also made significant progress in bioinformatics, drug discovery \cite{DrugDiscovery} and material sciences.
...
\section{Two-party Secure Computation for Basic Operations}
    ...
    simulator $\mathcal{S}im$ that implements $\mathtt{comparison}$ protocol.
    ...
    parameters used in the two $\mathtt{multiplication}$ protocols
    ...
    \begin{itemize}
        ...
        \item 5. running $\mathcal{P}_{0}$ on input $(\langle z\rangle ^{0},\langle z\rangle ^{1},\llbracket u \rrbracket ^{0})$.
    ...
    number. This is clearly true since $(\langle x\rangle^{1},\langle a\rangle_{1}^{1},\langle b\rangle_{1}^{1},
    \langle a\rangle_{2}^{1},\langle b\rangle_{2}^{1})$ is unknown to $\mathcal{P}_{0}$ in the real world.
    ...
    Since $\mathcal{P}_{1}$ gets less information in the $\mathtt{comparison}$ protocol than $\mathcal{P}_{0}$ gets,
    this protocol is secure for both party.
