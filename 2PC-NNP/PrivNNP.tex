...
\section{Two-party Secure Computation for Basic Operations}
    ...
    Since $\mathtt{comparison}$ protocol involves two $\mathtt{multiplication}$ protocol,
    intermediate parameters are marked with different subscripts: $1$ and $2$.
    % $(\langle a\rangle_{1},\langle b\rangle_{1},\langle c\rangle)_{1}$
    % and $(\langle a\rangle_{2},\langle b\rangle_{2},\langle c\rangle)_{2}$
    ...
    \begin{itemize}
        ...
        \item 6. randomly chooses $(\langle e\rangle_{2}^{1},\langle f\rangle_{2}^{1})$ and sends them to $\mathcal{P}_{0}$.

        \item 7. randomly chooses $\langle s\rangle^{1}$ s.t. $\langle s\rangle ^{0}+\langle s\rangle ^{1}=s$
        \item 8. outputs whatever $\mathcal{P}_{0}$ outputs.
    \end{itemize}

    Next, we show that the view Sim simulates for $\mathcal{P}_{0}$ is indistinguishable
