...
\section{Introduction}
    ...
    \begin{itemize}
        \item \textbf{PrivNNP}.
        We present PrivNNP, a two-party computation framework
        for evaluating neural network predictions efficiently.

        The building block of our work is a brand new 2PC-$\mathtt{comparison}$ protocol,
        which does not utilize the asymmetric cryptographic operations or the bit-level operations
        and only consists of three rounds of interactions to calculate the sign of the secret input.
        ...
        We theoretically prove that our system satisfy the
        security definition of the two-party computation.

        \item \textbf{Experiment}.
        To demonstrate the outstanding performance of our work,
        we conduct PrivNNP on the classical neural network model Resnet50 and
...
\section{Related Work}
    ...
    The most common idea is to utilize fully homomorphic encryption  \cite{SEAL} \cite{Homomorphic1} which supports multiplication and addition on ciphertext.
    ...
    Furthermore, some proposals that only using additive homomorphic encryption are presented \cite{Homomorphic2}.
    ...
    On a very high level, MPC multiplication is easy to be implemented by the Beaver triple \cite{EfficientMultipartyProtocols}
    ...
    Both ABY and ABY3 involve the asymmetric cryptographic operations
...
\section{Preliminaries}
    ...
    In addition, we denote a matrix using capital letters (e.g. $A$) and denote
    a vector as $\overrightarrow{\mathbf{v}}=(v_{0},v_{1},...,v_{n-1})$.
    ...
    \begin{figure}
        ...
        \begin{tabular}{lcl}
            ...
            &
            &
            mul-triple:$ (\langle \mathbf{A}\rangle ^{1},\langle b\rangle ^{1},\langle c\rangle ^{1})\}$
            \\
            \textbf{Procedure}:\\
            $\mathcal{P}_{0}$ &  &  $\mathcal{P}_{1}$ \\
    ...
    \textbf{Security analysis}:
       ...
       Following this idea, the whole process of computing $\langle maximum\rangle  = \langle \mathsf{max}(\langle x\rangle,\langle y\rangle)\rangle$ is given below.
       \begin{figure}
        \resizebox{0.48\textwidth}{!}{
        \begin{tabular}{lcl}
            \hline
        ...
        \caption{2PC-$\mathsf{max}$}
        \label{2PC-max$}
    \end{figure}
    2PC-comparison protocol is shown in Figure \ref{2PC-comparison}.

       Furthermore, we explore variations of $\mathsf{max}(x,y)$.
       Firstly, we talk about $\mathsf{max}(\overrightarrow{\mathbf{x}})=\mathsf{max}(x_{0},x_{1},...x_{n})$. Observing that
       \begin{align*}
       ...
       \end{align*}
       So the maximum of vector $\overrightarrow{\mathbf{x}}$ can be computed by recursively running $\mathtt{comparison}$ protocol.
       ...
       with dividing point $-\infty =d_{0}<d_{1}<...<d_{j}<...<d_{n}=+\infty$.
       ...
       So we can conclude two circumstances as $\mathsf{F}(x)=\sum_{j=1}^{n}v_{j}\cdot \mathsf{f}_{j}(x)$.
    ...
    \end{figure}
    The 2PC-piecewise function protocol is shown in Figure \ref{2PC-piecewise}.
    ...
    \subsection{System framework}
       ...
       directly take the role of $\mathcal{P}_{0}$ and $\mathcal{P}_{1}$ to run PrivNNP.
    ...
    \begin{figure*}[htbp]
        ...
        \centering
        \includegraphics[width=10cm]{new_framework.png}
    ...
    We design the 2PC schemes for these basic layers.
    ...
    Here we explore one  activation functions: $\mathsf{ReLU}()$, a typical continuous piecewise function.
