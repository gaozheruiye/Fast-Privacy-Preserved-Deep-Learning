...
\section{Two-party Secure Computation for Basic Operations}
    ...
    Our security proof follows the ideal-world/real-world paradigm.
    In real-world, parties interact following the protocol,
    while in ideal world, parties have access to an
    honest party $\mathcal{H}$ that implements $\mathtt{comparison}$ protocol.
    Environment $\varepsilon $  chooses the
    inputs to the parties.
    We aim at showing that the view of adversary
    in real-wold is indistinguishable to that in ideal-world.
    Since the $\mathtt{comparison}$ protocol involves two $\mathtt{multiplication}$ protocol,
    intermediate parameters are marked with different subscripts: $1$ and $2$.
    ...
    \begin{itemize}
        ...
        from $\varepsilon $.
        $\mathcal{S}im$ sends $\langle x\rangle ^{0}$ to $\mathcal{H}$ and gets the result $s$.
        ...
        \item 4. randomly chooses $\langle z\rangle ^{1}$ s.t. $sign(\langle z\rangle ^{0}+\langle z\rangle ^{1})=s$.
        and sends $\langle z\rangle ^{1}$ to $\mathcal{P}_{0}$.

        \item 5. running $\mathcal{P}_{0}$ on input $(\langle z\rangle ^{0},\llbracket u \rrbracket ^{0})$ and
        recieves $(\langle e\rangle_{1}^{0},\langle f\rangle_{1}^{0})$.

        \item 6. running $\mathcal{P}_{0}$ on input $\llbracket s \rrbracket^{0}$,
        $(\langle a\rangle_{2}^{0},\langle b\rangle_{2}^{0},\langle c\rangle_{2}^{0})$
        ...
        \item 8. randomly chooses $\langle s\rangle^{1}$ s.t. $\langle s\rangle ^{0}+\langle s\rangle ^{1}=s$
        \item 9. outputs whatever $\mathcal{P}_{0}$ outputs.
    \end{itemize}

    The view $\mathcal{S}im$ simulates for $\mathcal{P}_{0}$ is indistinguishable
    from the view of $\mathcal{P}_{0}$ interacting in the real execution.
    The view of $\mathcal{P}_{0}$ in the real execution is
    ...
    $ while its view in the ideal
    execution is
    $(\langle e\rangle_{1}^{1}:random,
    \langle f\rangle_{1}^{1}:random,)
    \langle z\rangle_{1}^{1}: $ a random number s.t. $sign(\langle z\rangle ^{0}+\langle z\rangle ^{1})=s$
    $(\langle e\rangle_{2}^{1}:random,
    \langle f\rangle_{2}^{1}:random),
    $
    So we only need to show that any
    element is indistinguishable from a random
    number. This is clearly true since any element in the ideal world is randomly chosen.
